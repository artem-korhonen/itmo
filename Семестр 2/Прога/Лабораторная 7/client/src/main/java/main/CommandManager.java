...
public class CommandManager {
      ...
      public CommandManager(ServerManager serverManager, Console console) {
            this.serverManager = serverManager;
            this.console = console;
            this.logged = false;
            this.commands.put("info",
                        new InfoCommand("info", "вывести информацию о коллекции", this.serverManager));
            ...
            this.commands.put("insert",
                        new InsertCommand("insert", "добавить новый элемент с заданным ключом",
                                    this.serverManager, this.console));
            this.commands.put("update", new UpdateCommand("update",
                        "обновить значение элемента коллекции, id которого равен заданному", this.serverManager, this.console));
            this.commands.put("remove_key",
                        new RemoveKeyCommand("remove_key", "удалить элемент из коллекции по его ключу",
                                    this.serverManager));
            this.commands.put("clear", new ClearCommand("clear", "очистить коллекцию", this.serverManager));
            this.commands.put("execute_script", new ExecuteScriptCommand("execute_script",
                        ...
                        this));
            this.commands.put("exit", new ExitCommand("exit", "завершить программу", this.console));
            this.commands.put("remove_greater", new RemoveGreaterCommand("remove_greater",
                        "удалить из коллекции все элементы, превышающие заданный", this.serverManager, this.console));
            this.commands.put("replace_if_greater", new ReplaceIfGreaterCommand("replace_if_greater",
                        "заменить значение по ключу, если новое значение больше старого", this.serverManager, this.console));
            ...
            this.commands.put("print_field_descending_governor",
                        new PrintFieldDescendingGovernorCommand("print_field_descending_governor",
                                    ...
                                    this.serverManager));
            this.commands.put("register", new RegisterCommand("register", "создать новый аккаунт пользователя", this, this.serverManager, this.console));
            this.commands.put("login", new LoginCommand("login", "войти в аккаунт пользователя", this, this.serverManager, this.console));
            this.commands.put("help", new HelpCommand("help", "вывести справку по доступным командам", this));
      }
      ...
      public void executeCommand(String input) {
            ...
            if (command != null) {
                  if (commandWords[0].equals("login") || commandWords[0].equals("register") || commandWords[0].equals("exit") || this.logged) {
                        command.execute(commandWords.length == 2 ? commandWords[1].split(" ") : new String[] {});
                  } else {
                        System.out.println("You need to log in your account or register");
                  }
            } else {
