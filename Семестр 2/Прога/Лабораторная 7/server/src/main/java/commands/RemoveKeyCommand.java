    ...
    public Response execute(Request request) {
        ...
        if (collectionManager.getCollection().keySet().contains(cityId)) {
            if (collectionManager.removeElementCollection(cityId)) {
                return new Response(List.of(collectionManager.getCollection().get(cityId).toString() + " has been removed"));
            } else {
                return new Response(List.of("Error: " + collectionManager.getCollection().get(cityId).toString() + " hasn't been removed (database)"));
            }
        } else {
